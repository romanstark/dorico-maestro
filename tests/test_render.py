...
# The golden fixture from the interface contract (§2.3): two bars, two-staff
# piano, C major 4/4.
...
# The byte-identical expected output of plan_flow(WORKED_EXAMPLE)[0] (§3.5).
GOLDEN: list[str] = [
    "NoteInput.Enter",
    "NoteInput.MoveUpTop",
...
def test_same_articulation_is_not_retoggled_between_notes() -> None:
    ...
    # once after the last — never toggled in between.
    spec = score_from_dict(
        {
            "parts": [
                {
...
async def test_import_musicxml_sends_raw_forward_slash_path(tmp_path: Any) -> None:
    ...
    # Fires the parameterised open exactly once — never the picker-risking bare rung.
    assert len(client.sent) == 1
    sent = client.sent[0]
    assert sent.startswith("File.Open?File=")
    assert "FilterID=MusicXMLImportFilter" in sent
    # The path goes on the wire raw with forward slashes — never percent-encoded
