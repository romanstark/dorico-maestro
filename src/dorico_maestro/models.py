...
class CmdStatus(str, Enum):
    """Integration status for a catalogued Dorico command.

    Status tracking reflects live testing against Dorico instances:
    - VERIFIED: Command executed and intended effect was confirmed.
    - REACHABLE: Command is accepted and ungated, but effect was not observed.
    - UNAVAILABLE: Refused with kUnknownCommand on the tested license tier.
    - BROKEN: Command reached Dorico and failed unexpectedly.
    - UNTESTED: Present in catalog but not yet probed live.
    """

    VERIFIED = "verified"          # ran and the effect was observed
    REACHABLE = "reachable"        # present and ungated, effect not observed
    UNAVAILABLE = "unavailable"    # kUnknownCommand here: absent, or gated by this edition
    BROKEN = "broken"              # reached Dorico and failed for a reason that is not the licence
...
class Response:
    ...
    kOK confirms command acceptance by the queue, not that the musical effect
    completed. Verification must occur via status deltas or score inspection.
    """
