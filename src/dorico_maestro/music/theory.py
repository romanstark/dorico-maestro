"""Music-theory helpers built on :mod:`music21`.

Pure musical analysis — this module never touches Dorico's transport. It answers
three questions a composition partner keeps asking:

* :func:`analyze_chord` — what *is* this stack of pitches? (root, quality, symbol)
* :func:`suggest_progression` — give me a diatonic chord progression in a key.
* :func:`note_in_range` — can *this* instrument actually play *that* pitch?

It also offers the deeper analysis a composition partner keeps reaching for: key
detection (:func:`detect_key`), roman-numeral / functional harmony
(:func:`roman_numeral_analysis`), parallel-perfect and voice-leading checks
(:func:`find_parallels`, :func:`check_voice_leading`), cadence and next-chord
suggestions (:func:`suggest_cadence`, :func:`suggest_next_chord`), instrument
range checking (:func:`check_ranges`) and first-species counterpoint
...
def _extract_lines(spec: ScoreSpec) -> list[_Line]:
    ...
    rests break nothing — they are simply omitted from the line.
    """
    lines: list[_Line] = []
    for pindex, part in enumerate(spec.parts):
        for sindex, staff in enumerate(part.staves):
...
def _vl_issue(
    ...
    """Build a voice-leading issue dict with a ``{part, staff, voice, index}`` location."""
    return {
        "severity": severity,
        "rule": rule,
        "message": message,
