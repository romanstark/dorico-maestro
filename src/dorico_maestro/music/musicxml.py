"""MusicXML generation and inspection via :mod:`music21`.
...
----------------------------------------------------
``notes`` is either a **sequence** of note specs (one part, named ``"Melody"``)
or a **mapping** of part name -> sequence of note specs (multiple parts). Each
note spec is one of:
...
def score_to_musicxml(spec: ScoreSpec, out_path: str | Path) -> str:
    """Write a :class:`~dorico_maestro.music.score.ScoreSpec` to a MusicXML file.
...
def _split_spec(spec: Any, default_dur: str | float) -> tuple[list[str], Any, str | None]:
    """Split a note spec into ``(pitches, duration, lyric)``; ``pitches`` empty = rest."""
    if spec is None:
...
def _build_note_object(spec: Any, default_dur: str | float) -> note.GeneralNote:
    """Create a music21 :class:`Note`, :class:`Chord` or :class:`Rest` from a spec."""
...
def _coerce_key(value: str | int) -> key_mod.KeySignature:
    """Coerce a key spec (name, or integer number of sharps/flats) to a key object."""
...
def _ambitus(part: stream.Part) -> dict[str, Any] | None:
    """Lowest/highest sounding pitch of a part (chord symbols excluded)."""
    pitches = []
...
def _key_name(score: stream.Score) -> str | None:
    """Notated key if present, else music21's best analysis, else ``None``."""
