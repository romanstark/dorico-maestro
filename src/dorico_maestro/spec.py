"""Command specifications and the generic command-string builder.

A :class:`CommandSpec` describes *one* Dorico command as data — its id, the
parameters it accepts, and some flags the executor cares about. :func:`build`
turns a spec plus concrete arguments into the wire string Dorico expects
(``Namespace.Command?Key=Value&Key2=Value2``); :func:`validate` enforces the
spec's contract before anything is sent.

This module is pure data + string assembly: it never touches the transport or
music21. See ``docs/architecture.md``.
"""
