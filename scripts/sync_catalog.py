"""Sync ``commands.yaml`` with a Dorico ``keycommands.json``.

Reads the command IDs that one Dorico install ships and merges them into
the catalog.
...
* command IDs present in the catalog but *absent* from ``keycommands.json`` are
  ...
  *key-bindable* subset of Dorico's command set -- ``File.Export`` and
  ``Print.ExportCurrentLayoutAsPDF`` are real, dispatchable and verified, and
  neither appears in it. Expect this report to name them every run.
